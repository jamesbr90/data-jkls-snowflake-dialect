...
class SnowflakeDialect extends knex_1.default.Client {
    constructor(config = {}) {
    ...
    acquireRawConnection() {
        return new Promise((resolver, rejecter) => {
            ...
            connection.on('error', (err) => {
                console.log('error occurred connecting to snowflake', err);
                connection.__knex__disposed = err;
            });
            connection.connect((err) => {
                if (err) {
                    // if connection is rejected, remove listener that was registered above...
                    console.log('error connecting to snowflake', err);
    ...
    postProcessResponse(result, queryContext) {
        ...
        const lowercaseAttrs = (row) => {
            return lodash_1.fromPairs(lodash_1.toPairs(row).map(([key, value]) => [key.toLowerCase(), value]));
        };
        if (result.rows) {
            return Object.assign(Object.assign({}, result), { rows: result.rows.map(lowercaseAttrs) });
        }
